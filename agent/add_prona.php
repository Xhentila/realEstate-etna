...
<?php
    if(isset($_POST['p-submit']))
    {
        if(!empty($_POST['p_space']) && !empty($_POST['p_certif']) && !empty($_POST['p_price']) && !empty($_POST['p_contact']) && !empty($_POST['p_st']) && !empty($_POST['p_nr']) && !empty($_POST['p_zona']) && !empty($_POST['p_title']) &&!empty($_POST['p_pershkrimi']))
        {
            ...
            //shiko cili agjent po e shton per momentin sepse me duhet id
            $sql1 = "SELECT * FROM `agents` WHERE `agents`.`nipt` = '$agent_nipt'";
            $agent = mysqli_query($con, $sql1);
            if($agent){            
                while ($agent_data = mysqli_fetch_array($agent)){
                    $_SESSION['id'] = $agent_data['id'];
                }
            ...
            if($row == 0 ){
                ...
                // perfundoje per tabelen e re qe keni krijuar. 

                header("Location: ../agent/add_prona.php?".$agent_id."?Success=added");
            }else{
                $_SESSION['msg'] = "Property was not added  ";
                $_SESSION['alert']= "alert alert-success";
                header("Location: ../agent/add_prona.php?".$_SESSION['nipt']."?warning=alreadyadded");
            }

            $prona_id = "";
            $adresa_id = "";
            $sql = "SELECT `prona_id` FROM `prona` ORDER BY `prona_id` DESC LIMIT 0,1 ";
            $sql_row = mysqli_query($con, $sql);
            $sql_row_check =  mysqli_num_rows($sql_row);
            if($sql_row_check != 0){
                while ($sql_result = mysqli_fetch_array($sql_row) ){
                    $prona_id .= $sql_result['prona_id'];
                }
            }

            
            $sqli = "SELECT `adresa_id` FROM `adresa` ORDER BY `adresa_id` DESC LIMIT 0,1 ";
            $sqli_row = mysqli_query($con, $sqli);
            $sq_row_check = mysqli_num_rows($sqli_row);
            if($sq_row_check != 0){
                while ($sqli_result = mysqli_fetch_array($sqli_row) ){
                    $adresa_id .= $sqli_result['adresa_id'];
                }
            }

            $stmtss = $con->prepare("INSERT INTO `prona_adresa`(`prona_id`,`qyteti_id`,`adresa_id`)VALUES (?,?,?)");
                $stmtss-> bind_param("iii", $prona_id, $p_qyteti, $adresa_id );
                $stmtss->execute();
                $stmtss->close();

            echo $prona_id;
        } else {
            header("Location: ../agent/add_prona.php?".$_SESSION['nipt']."?error");
        }
    }
...
<main>
    ...
    <div class="container center-div">
            ...
            <div class="row">
                <div class="col-md-12 " id="addpropform"  style="margin-top: 30%">
                    <p >Shtoni pronat tuaja ketu!</p>
                    <hr />
                    <form action="add_prona.php" method="POST" enctype="multipart/form-data">
